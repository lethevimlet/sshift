...
// Read the refactored WebSocket handlers
const tabsPath = path.join(__dirname, '..', 'server', 'endpoints', 'ws', 'tabs.js');
...
const stickyDisconnectMatch = tabsCode.match(/if \(tab\.sticky\)[\s\S]*?console\.log\('\[TAB\] Sticky session remains open on server:'/);
...
const nonStickyDisconnectMatch = tabsCode.match(/\/\/ For non-sticky sessions[\s\S]*?const gracePeriod = 5000/);
...
const tabCloseMatch = tabsCode.match(/\/\/ When user explicitly closes a tab, close the session immediately[\s\S]*?\/\/ regardless of sticky setting/);
if (tabCloseMatch) {
...
const tabCloseNoGrace = tabsCode.match(/console\.log\('\[TAB\] Tab explicitly closed by user, closing session:'/);
if (tabCloseNoGrace) {
...
const clearTimerMatch = tabsCode.match(/if \(tab\.sticky\)[\s\S]*?clearCloseTimer\(sessionId\)/);
if (clearTimerMatch) {
...
const disconnectHandler = tabsCode.match(/handleTabDisconnect[\s\S]*?\n\}/);
...
const disconnectNonStickyClose = tabsCode.match(/\/\/ For non-sticky sessions[\s\S]*?sshManager\.disconnect\(sessionId\)/);
