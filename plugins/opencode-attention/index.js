 ...
 * is waiting for the user (reply complete, or a permission/question dialog).
 *
 ...
 *      while its UI signature (the "OpenCode" wordmark in the status bar /
 *      splash) is visible in the CURRENT viewport of the headless terminal.
 *      When the user exits back to a shell, the signature disappears and,
 *      after a few misses, all state resets — so a shell prompt like "❯ "
 *      can never flash the tab (the old sticky flag caused exactly that).
 *
 *   2. "Working" is tracked from two signals:
 *        - working indicators visible in the viewport footer (braille
 *          spinner glyphs, legacy ⬝■▣ spinners, "esc to interrupt"), and
 ...
 *      confirmation dialog both look like this transition, so no brittle
 *      prompt regexes are needed.
 *
 ...
 * "press enter" or "❯ " routinely appear inside chat prose, code and shell
 * prompts and caused constant false flashes. Custom `patterns` from the
 * config are still honored, but they are only evaluated while OpenCode is
 * on screen and not working.
 *
 ...
 *   - appMissLimit: number      - Consecutive viewport checks without the app
 *                                 signature before state resets (default: 3)
 *   - appPatterns: string[]     - Regexes replacing the default app signature
...
class OpenCodeAttentionPlugin {
  constructor(ctx, config = {}) {
    ...
    // App signature: the OpenCode TUI always renders its wordmark (status
    // bar "OpenCode x.y.z", splash logo). Case-SENSITIVE on purpose so
    // lowercase shell text (`cd opencode`, paths, package names) can't arm
    // the engine — and even a false positive here only ARMS it; a flash
    // still requires a real working → idle transition.
    this._appPatterns = this._compileList(
      config.appPatterns || ['OpenCode'],
      'appPatterns',
      '' // case-sensitive
    );
  ...
  _getSessionState(sessionId) {
    if (!this._sessionState.has(sessionId)) {
      this._sessionState.set(sessionId, {
        ...
        appMissStreak: 0,         // consecutive checks without the signature
  ...
  _evaluate(sessionId) {
    ...
    if (appOnScreen) {
      ...
      if (s.appMissStreak >= this.config.appMissLimit) {
        // OpenCode left the screen (user exited to shell, switched apps).
